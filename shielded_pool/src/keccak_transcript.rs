...
/// Keccak transcript aligned with `snark-verifier` EVM transcript semantics.
///
/// This matches:
/// - absorb: concatenation of encoded inputs
/// - squeeze: `keccak256(state)` with `state || 0x01` if length is exactly 32
/// - state transition: `state = squeeze_output`
...
impl Hashable<KeccakTranscript> for Fq {
...
impl Sampleable<KeccakTranscript> for Fq {
    fn sample(hash_output: Vec<u8>) -> Self {
        assert_eq!(hash_output.len(), 32, "KeccakTranscript outputs 32 bytes");
...
impl Hashable<KeccakTranscript> for G1Projective {
    fn to_input(&self) -> Vec<u8> {
        ...
        let coordinates = match Option::<midnight_curves::Coordinates<MidnightG1Affine>>::from(
            ...
            None => return vec![0u8; 2 * EVM_ENCODED_FP_BYTES],
        };
    ...
    fn to_bytes(&self) -> Vec<u8> {
        ...
        let coordinates = match Option::<midnight_curves::Coordinates<MidnightG1Affine>>::from(
            ...
            Some(coordinates) => coordinates,
            None => return vec![0u8; 2 * midnight_fp_num_bytes()],
        };
...
fn scalar_from_evm_bytes(bytes_be: &[u8]) -> io::Result<Fq> {
    ...
    Option::from(Fq::from_repr(repr)).ok_or_else(|| io::Error::other("invalid scalar encoding"))
}
...
fn fp_from_be_bytes(bytes_be: &[u8]) -> io::Result<MidnightFp> {
    let expected = midnight_fp_num_bytes();
    if bytes_be.len() != expected {
        return Err(io::Error::other(format!(
            "invalid field element length: expected {expected} bytes, got {}",
            bytes_be.len()
        )));
    }
    let mut le = bytes_be.to_vec();
    ...
    Option::from(MidnightFp::from_repr(repr))
        .ok_or_else(|| io::Error::other("invalid BLS12-381 base field encoding"))
}
