    ...
    L: Loader<C> + MidnightTruncatedChallengeOps<C>,
{
    ...
    // Cache one loaded shift scalar per rotation so we can factor denominators by shift constants.
    ...
    // Precompute z^k so lagrange denominators can use shift-only products times z^{-k}.
    ...
    // Per-point-set interpolation data used after one shared batch inversion.
    struct InterpTerm<S> {
        proof_eval: S,
        evals: Vec<S>,
        den: S,
        lagrange_start: usize,
        // Inverse index for z^{|set|-1} used when factoring lagrange denominators by shifts.
        z_pow_inv_idx: usize,
        diff_start: usize,
        has_lagrange: bool,
        den_idx: usize,
        ...
        point_sets.iter().zip(q_eval_sets.iter()).zip(proof.q_evals_on_x3.iter()).rev()
    {
        ...
        let shifts_scalars = shifts
            .iter()
            .map(|shift| {
                shift_scalars.get(shift).cloned().ok_or_else(|| {
                    Error::InvalidProtocol(format!("missing loaded shift {}", shift.0))
                })
            })
            .collect::<Result<Vec<_>, _>>()?;
        ...
        if has_lagrange {
            // Lagrange denominators in shift space: Π_{k != j} (s_j - s_k).
            ...
            // Multiply by z^{-(|set|-1)} after batch inversion to recover point-space denominator inverse.
            z_pow_inv_idx = den_pool.len();
            den_pool.push(z_powers[shifts_scalars.len() - 1].clone());
        }

        let diff_start = den_pool.len();
        // Store all (x3 - point_j) terms so their inverses can be reused in barycentric basis.
    ...
    for term in f_eval_terms {
        let r_eval = if term.evals.is_empty() {
            ...
            // Barycentric-style interpolation using cached denominator and inverse pools.
            term.evals.iter().enumerate().fold(loader.load_zero(), |acc, (j, eval)| {
                ...
                if term.has_lagrange {
                    ...
                    // Apply the factored z^{-(|set|-1)} component.
    ...
    let powers_x4 = loader.powers_with_challenge_policy(&proof.x4, proof.q_evals_on_x3.len() + 1);
