...
impl<S: SelfEmulation> VerifierGadget<S> {
    /// Assigns a verifying key where `transcript_repr` is a fixed constant.
    ///
    /// This is a compatibility helper for circuits that pin the verifier key
    /// inside the circuit instead of exposing `transcript_repr` as public input.
    pub fn assign_vk_to_fixed(
        ...
        let transcript_repr: AssignedNative<S::F> =
            self.scalar_chip.assign_fixed(layouter, transcript_repr_constant)?;
