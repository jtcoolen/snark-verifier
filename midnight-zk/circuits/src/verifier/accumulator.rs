...
impl<S: SelfEmulation> AssignedAccumulator<S> {
    ...
    /// collapsing each side to a single point.
    ///
    /// Contrary to [`Self::fully_collapse`], this expects fixed bases to be
    /// already assigned in-circuit.
    pub fn fully_collapse_with_assigned_fixed_bases(
        &self,
        layouter: &mut impl Layouter<S::F>,
        curve_chip: &S::CurveChip,
        fixed_bases: &BTreeMap<String, S::AssignedPoint>,
    ) -> Result<(S::AssignedPoint, S::AssignedPoint), Error> {
        let lhs_pt = self.lhs.eval_with_assigned_fixed_bases(layouter, curve_chip, fixed_bases)?;
        let rhs_pt = self.rhs.eval_with_assigned_fixed_bases(layouter, curve_chip, fixed_bases)?;
        Ok((lhs_pt, rhs_pt))
    }
